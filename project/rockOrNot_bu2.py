...
# Function for reading training data and labels
# First reads train_data.csv, then train_labels.csv and returns both
def readTraining(data_path,label_path):
...
#    print data_content
    ...
    with open(label_path, 'r') as csvfile:
        ...
        reader = csv.reader(csvfile, delimiter=',')
        for row in reader:
            data_labels.append([int(i) for i in row])
            if countingvar > countingvarlimit:
                break
            countingvar += 1
...
# Main
#readTraining(train_data_path,train_label_path)

label_count = {}
# Plot the labels of the training data, this part can be removed later
for label in readTraining(train_data_path,train_label_path):
